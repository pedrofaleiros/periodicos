...
async function main(start, end, forceUpdate) {
  ...
  for (let num = start; num <= end && num < data.length; num++) {
    ...
    var periodico = await prismaClient.portalIssnPeriodico.findUnique({
      where: { issn: issn },
    });

    if (issn === undefined) {
      console.log(chalk.red(`[${num}] - Não encontrado`));
    } else if (periodico !== null && !forceUpdate) {
      console.log(`[${num}] - Passando ${issn}`);
    } else {
